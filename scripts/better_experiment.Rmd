...
names(df) <- c('Size','Function','Execution time')
df
```
Calculate the execution time mean (sample mean) for each function
```{r}
library(dplyr)
sample_means <- df %>% group_by(Function) %>% summarise(Sn=mean(`Execution time`))
sample_means
```
Wow!! It seems that the parallel quicksort is the worst algorithm!
Let's confirm that using the Confidence Interval.